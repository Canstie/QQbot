...
- `~涩图 [数量]`：从共享的 `/download` 图库随机选取图片，合并成一份聊天记录发送。不填数量时随机选 1—5 张；指定数量时可选 1—100 张。遵循群启用策略和限流。全局 LRU 优先选择从未使用或最久未使用的图片，同批不重复；图库不足时发送实际数量。顺序持久化在 SQLite，读取失败或聊天记录发送未确认时会释放本次预留位置；图片从 MinIO 流式读取至临时目录，发送后清理。
