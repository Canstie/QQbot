...
每个知识库可独立绑定 DSAPI 模型、Thinking 模式、回复模式、最大输出 Token、最多 50 轮 AI 对话、
最多 100 条群聊上下文和 Temperature。还可开启按需联网检索，并指定一个群号与 QQ 号作为参考角色；
机器人会从保留 7 天的群记录中按当前话题检索相似说话示例，同一 QQ 的历史互动会优先匹配。
人物关系可按 `QQ号=关系说明` 逐行维护，只向模型注入当前说话人的对应关系，避免把称呼串给其他群友。
