...
- `~爆典all`：将当前群全部典图按最新保存优先整理成嵌套聊天记录，发回当前群；遵循群启用策略和限流。每份子记录最多 10 张、约 50 MiB，所有子记录合并为一份外层聊天记录，只调用一次群转发发送，不影响普通爆典计数。读取时校验大小和 SHA-256，失败图片跳过并提示；全部临时图片保留至外层发送结束再清理，因此临时磁盘需要容纳本次全部图片。等待 OneBot 返回最长 600 秒，不改变 LLBot 自身超时；失败时不自动重试，避免重复发送。
