...
- `~爆典all`：将当前群全部典图按最新保存优先分批发回当前群；遵循群启用策略和限流。每批最多 40 张、约 50 MiB，先逐张发送至 Bot 自己的私聊，再使用返回的真实消息 ID 原生合并转发到当前群，不嵌套、不再合成外层记录，不影响普通爆典计数。Bot 私聊中的图片消息保留，不自动撤回。读取时校验大小和 SHA-256，失败图片跳过并提示；每批临时图片在该批发送结束后清理。每次等待 OneBot 返回最长 600 秒，不改变 LLBot 自身超时；私聊发送失败、消息 ID 无效或群转发失败都会停止后续批次，不回退自定义节点转发、不自动重试，并区分本体发送失败和群转发失败。
