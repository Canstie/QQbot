...
- `~爆典all`：将当前群全部典图按最新保存优先分批发回当前群；遵循群启用策略和限流。每批最多 40 张、约 50 MiB，逐批发送独立的合并转发聊天记录，不嵌套、不再合成外层记录，不影响普通爆典计数。读取时校验大小和 SHA-256，失败图片跳过并提示；每批临时图片在该批发送结束后清理。每次等待 OneBot 返回最长 600 秒，不改变 LLBot 自身超时；失败时报告当前批次和已确认发送数量，停止后续批次且不自动重试，避免重复发送。
