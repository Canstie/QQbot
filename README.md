...
每个知识库可独立绑定 DSAPI 模型、Thinking 模式、最大输出 Token 和 Temperature；
...
- 转发 QQ 小程序到已启用群聊：自动提取标题和精简链接；小红书图片帖还会发送全部图片，图片只在发送期间临时缓存。
...
- `/download`：管理员引用普通图片消息或合并转发聊天记录后使用。Bot 会递归下载记录中的图片，以 SHA-256 内容哈希跨日期查重，并保存到 `downloadimage/YYYYMMDD/`。可通过 `QQBOT_DOWNLOAD_IMAGE_DIR` 修改根目录。
